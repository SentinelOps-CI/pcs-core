...
`examples/benchmark_ingest/` contains **producer-validated** `PcsBenchIngest.v0` bundles (one per owning repo), generated by `materialize_benchmark_producer_examples.py` from dialects or live LabTrust gallery runs. They are checked by `pcs examples check`, `pcs benchmark validate`, and `pcs conformance run --suite benchmark-ingest`.
