...
The LabTrust trust loop runs from LabTrust-Gym through `RuntimeReceipt.v0`, CertifyEdge `TraceCertificate.v0`, `ScienceClaimBundle.v0`, Provability Fabric `VerificationResult.v0`, `SignedScienceClaimBundle.v0`, and Scientific Memory import and rendering.

Canonical fixtures live in `examples/labtrust-release/` inside pcs-core, and downstream repositories sync this directory at a pinned pcs-core commit or prove canonical-hash equivalence to the manifest through the full materialize workflow instead of refreshing individual files in isolation.

| File | Role |
|------|------|
| `RELEASE_FIXTURE_MANIFEST.json` | Artifact hashes and five producer commits |
| `release_manifest.v0.json` | `ReleaseManifest.v0` with digest signing |
...
Continuous integration runs `pcs validate-release-chain` on every push.
...
| 4 | Every artifact uses real commits without `local_dev`, zero commits, or placeholder commits |
| 5 | `runtime_receipt.json` `source_commit` equals `manifest.labtrust_gym_commit` |
...
| 23–26 | Scientific Memory import with `passed`, `strict=true`, `allow_legacy=false`, `bundle_shape=pcs_core` |
...
Constants live in `pcs_core.release_canonical`.
