...
Each subdirectory is a copy of the canonical [`../labtrust-release/`](../labtrust-release/) chain with one intentional violation, and `pcs validate-release-chain` must fail with the listed code.
...
Regenerate from the canonical tree.

```bash
python python/scripts/materialize_invalid_release_fixtures.py
```

Validator reference is [docs/labtrust-release-fixtures.md](../../docs/labtrust-release-fixtures.md).
